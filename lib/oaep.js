...
function privateDecryptOaep(privateKey, ciphertext, options) {
  ...
  try {
    ...
    // Random bytes or malformed ciphertext can exceed the modulus and fail
    // before OAEP decode even starts. Don't reveal which check failed.
    throw decodingError();
  }
  ...
  for (var i = hLen; i < db.length; i++) {
    var isOne = (db[i] ^ 0x01) === 0 ? 1 : 0;
    var isZero = db[i] === 0 ? 1 : 0;
