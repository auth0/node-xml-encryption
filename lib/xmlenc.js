...
function decrypt(xml, options, callback) {
  ...
  try {
    var doc = typeof xml === 'string' ? new xmldom.DOMParser().parseFromString(xml, options.mimeType ?? xmldom.MIME_TYPE.XML_APPLICATION) : xml;
...
function decryptKeyInfo(doc, options) {
  if (typeof doc === 'string') doc = new xmldom.DOMParser().parseFromString(doc, options.mimeType ?? xmldom.MIME_TYPE.XML_APPLICATION);
