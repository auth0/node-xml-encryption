...
function encryptKeyInfoWithScheme(symmetricKey, options, padding, mgf1Hash, callback) {
  ...
  try {
    ...
    if (isOAEP && mgf1Hash !== oaepHash) {
      // Node cannot set MGF1 separately from the OAEP digest.
...
function encryptKeyInfo(symmetricKey, options, callback) {
  ...
  switch (options.keyEncryptionAlgorithm) {
    ...
    case 'http://www.w3.org/2009/xmlenc11#rsa-oaep': {
      ...
      } else if (!['sha1', 'sha224', 'sha256', 'sha384', 'sha512'].includes(mgf1Hash)) {
...
function decryptKeyInfo(doc, options) {
  ...
  switch (keyEncryptionAlgorithm) {
    case 'http://www.w3.org/2001/04/xmlenc#rsa-oaep-mgf1p':
      ...
      return decryptKeyInfoWithScheme(encryptedKey, options, crypto.constants.RSA_PKCS1_OAEP_PADDING, oaepHash, 'sha1');
    ...
    case 'http://www.w3.org/2009/xmlenc11#rsa-oaep': {
      ...
      return decryptKeyInfoWithScheme(encryptedKey, options, crypto.constants.RSA_PKCS1_OAEP_PADDING, oaepHash, mgf1Hash);
    }
...
function decryptKeyInfoWithScheme(encryptedKey, options, padding, oaepHash, mgf1Hash) {
  const key = Buffer.from(encryptedKey.textContent, 'base64');
  // Node cannot set the MGF1 digest separately, so only fall back to the JS
  // implementation when it actually differs. Everything else stays in OpenSSL.
  if (padding === crypto.constants.RSA_PKCS1_OAEP_PADDING && !mgf1Hash) {
    throw new Error('mgf1Hash is required for OAEP padding');
  }
  if (padding !== crypto.constants.RSA_PKCS1_OAEP_PADDING || mgf1Hash === oaepHash) {
    const decrypted = crypto.privateDecrypt({ key: options.key, padding, oaepHash }, key);
    return Buffer.from(decrypted, 'binary');
  }
  return oaep.privateDecryptOaep(options.key, key, { oaepHash, mgf1Hash });
}
