...
function encrypt(content, options, callback) {
  ...
  async.waterfall([
    ...
    function encrypt_content(symmetricKey, cb) {
      switch (options.encryptionAlgorithm) {
        case 'http://www.w3.org/2001/04/xmlenc#aes128-cbc':
          encrypt_aes('aes-128-cbc', symmetricKey, content, options.input_encoding, function (err, encryptedContent) {
            if (err) return cb(err);
            cb(null, symmetricKey, encryptedContent);
          });
          break;
        case 'http://www.w3.org/2001/04/xmlenc#aes256-cbc':
          encrypt_aes('aes-256-cbc', symmetricKey, content, options.input_encoding, function (err, encryptedContent) {
            if (err) return cb(err);
            cb(null, symmetricKey, encryptedContent);
          });
          break;
...
function encrypt_aes(algorithm, symmetricKey, content, encoding, callback) {
  // create a random iv for aes-XXX-cbc
