...
function decrypt(xml, options, callback) {
  ...
  try {
    ...
    switch (encryptionAlgorithm) {
      case 'http://www.w3.org/2001/04/xmlenc#aes128-cbc':
        return decryptWithAlgorithm('aes-128-cbc', symmetricKey, 16, encrypted, callback);
      case 'http://www.w3.org/2001/04/xmlenc#aes256-cbc':
        return decryptWithAlgorithm('aes-256-cbc', symmetricKey, 16, encrypted, callback);
      case 'http://www.w3.org/2001/04/xmlenc#tripledes-cbc':
        return decryptWithAlgorithm('des-ede3-cbc', symmetricKey, 8, encrypted, callback);
      default:
...
function decryptWithAlgorithm(algorithm, symmetricKey, ivLength, content, callback) {
  ...
  if (1 <= padding && padding <= ivLength) {
    ...
    return callback(new Error('padding length invalid'));
  }

  return callback(null, new Buffer(decrypted, 'binary').toString('utf8'));
}
