  ...
  try {
    ...
    switch (encryptionAlgorithm) {
      case 'http://www.w3.org/2001/04/xmlenc#aes128-cbc':
        ...
        decryptedBuf = decipher.update(encrypted.slice(16));
        decryptedBufLeft = decipher.final();
        if (decryptedBufLeft.length != 0) {
          //should not happen, we are not use stream here
          decryptedBuf = Buffer.concat([decryptedBuf, decryptedBufLeft]);
        }
        if (typeof options.autopadding === 'undefined' || options.autopadding === true) {
          padding = decryptedBuf[decryptedBuf.length - 1];
          if (padding <= 0x10 && padding >= 0x01) {
            decryptedBuf = decryptedBuf.slice(0, decryptedBuf.length - padding);
          } else {
            callback(new Error('padding length incorrect'));
          }
        }
        decrypted = decryptedBuf.toString('binary');
