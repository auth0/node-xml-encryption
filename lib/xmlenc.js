...
function decryptKeyInfo(doc, options) {
  ...
  let oaepHash = 'sha1';
  // DigestMethod is a child of the EncryptionMethod in use, so read it from
  // there: the two describe the same key. EncryptedKey is not always under
  // KeyInfo -- with EncryptedData/KeyInfo/RetrievalMethod it sits elsewhere in
  // the document -- and where several EncryptedKeys are present, each carries
  // its own digest.
